/**
 * Shared MotherDuck/DuckDB utilities for Vercel API routes
 *
 ...
 */
export async function initMotherDuck(): Promise<ReturnType<typeof duckdb.Database>> {
  if (db) return db
 ...
 */
export async function getConnection(): Promise<ReturnType<typeof duckdb.Database>> {
 ...
 */
export async function getAttackDistribution(): Promise<{ attack: string; count: number }[]> {
  return executeQuery(`
